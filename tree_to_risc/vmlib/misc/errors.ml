...
let iter_range f start finish =
  if Sys.file_exists !file then
    ...
    in
    loop 1
  else failwith "Utils.file not set"
...
let show_error ?(padding = 5) fmt ((ls, cs), (lf, cf)) =
  let show_line i l =
    if i = ls && i = lf then
  ...
  in
  iter_range show_line (max (ls - padding) 1) (lf + padding)

let pp_range fmt ((ls, cs), (lf, cf)) =
...
let from_loc fmt (start, finish) =
  ...
  if !Utils.error_verbose then
    show_error fmt ((start.pos_lnum, col_s), (finish.pos_lnum, col_f));
  Format.fprintf fmt "%!"
